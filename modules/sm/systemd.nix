...
#
# enable 开关：config.modules.sm.systemd.enable（默认 false）
{
  config,
  lib,
  ...
}:
{
  ...
  options.modules.sm.systemd.enable = lib.mkEnableOption "systemd 配置（journald/logind）";

  config = lib.mkIf config.modules.sm.systemd.enable {
