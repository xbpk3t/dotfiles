...
in
{
  ...
  readReport =
    ...
    in
    if reportPath == null then null else builtins.fromJSON (builtins.readFile reportPath);

  # What：约定每台主机的 report 放在 hosts/<host>/facter.json。
  # Why：让 report 与对应 host 配置同目录收敛，维护和 review 都更直观。
  reportPathForHost = hostName: pathIfExists (relativeToRoot "hosts/${hostName}/facter.json");
}
