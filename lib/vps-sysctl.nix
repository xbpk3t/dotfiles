...
let
  ...
  mkSysctl =
    ...
    let
      ...
      cores = if cpuCores == null then 1 else cpuCores;
      ...
      tcpRmem = if has10g then "4096 262144 ${toString rmemMax}" else "4096 131072 ${toString rmemMax}";

      tcpWmem = if has10g then "4096 262144 ${toString wmemMax}" else "4096 131072 ${toString wmemMax}";
      ...
      # NIC/吞吐相关队列
      netdevBacklog = if has10g then 250000 else 30000;
      netdevBudget = clamp 300 1000 (cores * 20);
      netdevBudgetUsecs = clamp 2000 16000 (if nicMbps <= 1000 then 4000 else 8000);
    ...
    in
    {
      ...
      "kernel.numa_balancing" = if cores >= 32 then 1 else 0;
      ...
      "vm.swappiness" = if isSsd then 10 else 20;

      # 脏页比例：内存越大越保守，避免大写入导致的卡顿
      "vm.dirty_ratio" = if ramGiB >= 16 then 10 else 15;
      "vm.dirty_background_ratio" = if ramGiB >= 16 then 3 else 5;
