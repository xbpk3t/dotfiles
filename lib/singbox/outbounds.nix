...
let
  ...
  toVlessOutbound =
    ...
    in
    if packetEncoding == null then base else base // { packet_encoding = packetEncoding; };
  ...
  toVmessOutbound =
    ...
    if !(s ? vmessWs) then
      ...
      in
      if domain == null || port == null || path == null then
        throw "singbox: servers.*.vmessWs.{domain,port,path} are required"
      else
        {
          type = "vmess";
          tag = mkTag "vmess" s;
  ...
  toHy2Outbound =
    ...
    if !(s ? hy2) then
      ...
      in
      if domain == null then
        throw "singbox: servers.*.hy2.domain is required"
      else
        {
          type = "hysteria2";
          tag = mkTag "hy2" s;
  ...
  toTuicOutbound =
    ...
    if !(s ? tuic) then
      ...
      if domain == null || port == null then
        throw "singbox: servers.*.tuic.{domain,port} are required"
      else
        {
          type = "tuic";
          tag = mkTag "tuic" s;
  ...
  toAnytlsOutbound =
    ...
    if !(s ? anytls) then
      ...
      in
      if domain == null || port == null then
        throw "singbox: servers.*.anytls.{domain,port} are required"
      else
        {
          type = "anytls";
          tag = mkTag "anytls" s;
...
builtins.seq flyingbirdPassword (
  outs
  ++ [
    # urltest 负责测速与自动选优
    {
