...
let
  ...
  toVmessOutbound =
    ...
    if !(s ? vmessWs) then
      ...
      if domain == null || port == null || path == null then
        throw "mihomo: servers.*.vmessWs.{domain,port,path} are required"
      else
        {
          name = mkTag "vmess" s;
          type = "vmess";
  ...
  toHy2Outbound =
    ...
    if !(s ? hy2) then
      ...
      in
      if domain == null then
        throw "mihomo: servers.*.hy2.domain is required"
      else
        {
          name = mkTag "hy2" s;
          type = "hysteria2";
  ...
  toTuicOutbound =
    ...
    if !(s ? tuic) then
      ...
      if domain == null || port == null then
        throw "mihomo: servers.*.tuic.{domain,port} are required"
      else
        {
          name = mkTag "tuic" s;
          type = "tuic";
  ...
  toAnytlsOutbound =
    ...
    if !(s ? anytls) then
      ...
      in
      if domain == null || port == null then
        throw "mihomo: servers.*.anytls.{domain,port} are required"
      else
        {
          name = mkTag "anytls" s;
          type = "anytls";
