...
- `nix` / `nix profile` / `nix store`: 原生 Nix CLI，负责 flake、profile、store 与 GC 等基础操作。
...
- `home/base/core/nh.nix`: `nix-index`、`nix-index-database`、`comma`、`nix-output-monitor`、`nvd`
- `modules/nixos/base/nix-tools.nix`: `nixos-cli`、原生 `nix.gc`
