...
已将 `nix-community/nixos-facter` 接入本仓库，主要用于自动采集并收敛硬件事实（CPU、磁盘、总线、显卡、内核模块等），从而大幅精简 `hardware.nix` 中原有的底层样板代码。
...
- 1、可以精简大量代码
- 把 `boot.initrd.availableKernelModules`、`kernelModules`、`hardware.cpu.*.updateMicrocode` 等硬件事实型配置交给 facter 自动处理，减少手写维护。
- `hardware.nix` 仅保留真正的运维策略和安装结果（如 `fileSystems`、`swapDevices`、电源策略、显卡 userspace runtime 等）。

---

> 职责边界：

- **facter 负责**：机器硬件事实（从真实主机自动采集）
- **不负责**：主机角色（role）、网络拓扑、服务策略、磁盘布局、电源管理策略等，这些仍需显式配置在 inventory 和 host 配置中。

---

> 当前状态：

- `nixos-homelab` 已接入真实 `facter.json` 并正式使用
- `nixos-vps`、`nixos-ws` 已准备好接入路径
- 接入方式：在 `hosts/<host>/hardware.nix` 中添加一行：
...
1. 在目标主机运行：`sudo nix run .#nixos-facter -- -o facter.json`
2. 将生成的 `facter.json` 放入 `hosts/<host>/` 目录
3. 在对应 `hardware.nix` 中声明 `reportPath`
4. 通过 `nix eval` + `nix flake check` 验证

---

**一句话总结**：

引入 `nixos-facter` 后，硬件事实自动收敛，`hardware.nix` 变得更干净、更聚焦策略，利大于弊，适合当前及未来多主机维护。
...
当前仓库已经移除了旧的 Haumea fixture 测试，因此 `facter` 这类 helper 逻辑的验收主要依赖针对性的 `nix eval` 与整体的 `nix flake check`。真正决定“这台机器能不能正常消费 report”的，仍然是 host 级的评估链路，而不是独立 fixture。
