...
let
  cfg = config.modules.devops.fastfetch;
in
{
  # host:
  #   modules.devops.fastfetch.enable = true;
  #
  # ⚠️ Darwin 上会拉入 apple-sdk-15.5（~481M 独有闭包），不需要 fetcher 时保持关闭。

  options.modules.devops.fastfetch = with lib; {
    enable = mkEnableOption "fastfetch (neofetch 替代) + 自定义配置";
  };
