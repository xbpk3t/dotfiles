...
let
  cfg = config.modules.ms.docker-cli;
in
{
  # host:
  #   modules.ms.docker-cli.enable = true;

  options.modules.ms.docker-cli = with lib; {
    enable = mkEnableOption "docker CLI + 配套镜像工具 (dive/slim/trivy/syft/grype 等)";
  };
