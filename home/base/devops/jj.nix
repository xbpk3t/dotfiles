...
in {
  ...
  # jjui TOML config as external file (same pattern as worktrunk.toml)
  # 用 builtins.readFile 而非 .source，避免 untracked 文件在 pure eval 时不在 store 中
  home.file.".config/jjui/config.toml".text = builtins.readFile ./jjui.toml;
}
